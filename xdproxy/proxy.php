...
//if($domainName == $RequestDomain) {
    ...
    error_log("Method is " . $method . " and the args are " . implode("; ", $args));
    error_log(print_r($args, true));
    $response = proxy_request($destinationURL, ($method == "GET" ? $_GET : $_POST), $method);
    $headerArray = explode("\r\n", $response[header]);

    if (strpos($headerArray[0], "301") !== false) {
	for ($i = 1; $i < count($headerArray); $i++) {
	    if (stripos($headerArray[$i], "Location") !== false) {
		error_log("Found a new redirect location: " . $headerArray[$i]);
		$headerArray[$i] = "Location: /xdproxy/proxy.php?destination=http://www.boardgamegeek.com/" . substr( $headerArray[$i], 11);
		error_log("New redirect line is " . $headerArray[$i]);
		break;
	    }
	}
    }
